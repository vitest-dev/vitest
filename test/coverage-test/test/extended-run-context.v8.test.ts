...
describe.each(['child process', 'worker thread'] as const)('%s', (runtime) => {
  ...
  test('{ autoAttachSubprocess: true } javascript source file', async () => {
    await runVitest({
      include: [filename],
  ...
  test('{ autoAttachSubprocess: true } pre-transpiled file', async () => {
    await runVitest({
      include: [filename],
  ...
  test(`{ autoAttachSubprocess: true } nested ${runtime}'s`, async () => {
    await runVitest({
      include: [filename],
  ...
  test('{ autoAttachSubprocess: true } when no child contexts are spawned', async () => {
    await runVitest({
