...
describe('Standard Schema', () => {
  ...
  describe('schemaMatching()', () => {
    ...
    test('should throw error for async schemas', () => {
      expect(() => expect('hello').toEqual(expect.schemaMatching(asyncStringSchema))).toThrowErrorMatchingInlineSnapshot(`
        TypeError {
          "message": "Async schema validation is not supported in asymmetric matchers.",
        }
      `)
    })
    ...
    test('should throw error for non-schema argument', () => {
      expect(() => expect.schemaMatching('not-a-schema')).toThrowErrorMatchingInlineSnapshot(`
        TypeError {
          "message": "SchemaMatching expected to receive a Standard Schema.",
        }
      `)
    })
