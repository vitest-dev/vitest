...
describe('vi.when()', () => {
  ...
  test('falls back to default implementation when not matching', () => {
  ...
  test('resolves matchers right to left', () => {
    const spy = vi.fn<Fn>()
  ...
  test('groups asymmetric matchers correctly', () => {
    const spy = vi.fn<Fn>(() => Number.NaN)
  ...
  test('works without having to re-declare arguments in a chain', () => {
    const spy = vi.fn<Fn>()
