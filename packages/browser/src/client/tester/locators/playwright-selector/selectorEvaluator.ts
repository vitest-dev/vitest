...
// copied without changes from https://github.com/microsoft/playwright/blob/4554372e456154d7365b6902ef9f3e1e7de76e94/packages/playwright-core/src/server/injected/selectorEvaluator.ts

import type { CSSComplexSelector, CSSComplexSelectorList, CSSFunctionArgument, CSSSimpleSelector } from './cssParser'
...
import { type ElementText, elementMatchesText, elementText, shouldSkipForTextMatching } from './selectorUtils'
...
const scopeEngine: SelectorEngine = {
  matches(element: Element, args: (string | number | Selector)[], context: QueryContext, _evaluator: SelectorEvaluator): boolean {
    if (args.length !== 0) {
  ...
  query(context: QueryContext, args: (string | number | Selector)[], _evaluator: SelectorEvaluator): Element[] {
    if (args.length !== 0) {
      throw new Error(`"scope" engine expects no arguments`)
    }
    const actualScope = context.originalScope || context.scope
...
const visibleEngine: SelectorEngine = {
  matches(element: Element, args: (string | number | Selector)[], _context: QueryContext, _evaluator: SelectorEvaluator): boolean {
    if (args.length) {
...
export function sortInDOMOrder(elements: Iterable<Element>): Element[] {
  interface SortEntry { children: Element[]; taken: boolean }
