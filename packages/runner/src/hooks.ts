...
import { getCurrentSuiteCollector, getRunner } from './suite'
...
export function beforeAll(fn: SuiteHooks['beforeAll'][0], timeout?: number) {
  const suite = getCurrentSuiteCollector()
  return suite.on('beforeAll', withTimeout(fn, timeout ?? getDefaultHookTimeout(), true))
}
export function afterAll(fn: SuiteHooks['afterAll'][0], timeout?: number) {
  const suite = getCurrentSuiteCollector()
  return suite.on('afterAll', withTimeout(fn, timeout ?? getDefaultHookTimeout(), true))
}

export function beforeEach<ExtraContext = {}>(fn: SuiteHooks<ExtraContext>['beforeEach'][0], timeout?: number) {
  const suite = getCurrentSuiteCollector<ExtraContext>()
  return suite.on('beforeEach', withTimeout(fn, timeout ?? getDefaultHookTimeout(), true))
}
export function afterEach<ExtraContext = {}>(fn: SuiteHooks<ExtraContext>['afterEach'][0], timeout?: number) {
  const suite = getCurrentSuiteCollector<ExtraContext>()
  return suite.on('afterEach', withTimeout(fn, timeout ?? getDefaultHookTimeout(), true))
}
