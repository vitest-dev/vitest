import type { WasmPlugin } from '@swc/core'
...
export class IstanbulCoverageProvider extends BaseCoverageProvider<ResolvedCoverageOptions<'swc'>> implements CoverageProvider {
  name = 'swc' as const
  ...
  onFileTransform(sourceCode: string, id: string, pluginCtx: any): { code: string; map: any } | undefined {
    ...
    // Use SWC to instrument the code
    const swcOptions: {
      filename: string
      sourceMaps: boolean
      jsc: {
  ...
  private async getCoverageMapForUncoveredFiles(coveredFiles: string[]) {
    ...
    for (const [index, filename] of uncoveredFiles.entries()) {
      ...
      // TODO: Use SWC to instrument uncovered files
