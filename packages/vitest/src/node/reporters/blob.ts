...
export class BlobReporter implements Reporter {
  ...
  async onTestRunEnd(testModules: ReadonlyArray<TestModule>, unhandledErrors: ReadonlyArray<SerializedError>): Promise<void> {
    ...
    const metadata: MergeReportMetadata | undefined = label ? { label } : undefined
...
export async function readBlobs(
  ...
  const promises = blobsFiles.map(async (filename) => {
    ...
    const [version, files, errors, coverage, executionTime, environmentModules, metadata] = parse(
      content,
    ) as MergeReport
    if (!version) {
      throw new TypeError(
    ...
    return { version, files, errors, coverage, file: filename, executionTime, environmentModules, metadata }
  })
  ...
  for (const blob of blobs) {
    const label = blob.metadata?.label
    if (!label) {
      continue
    }
