...
export class BlobReporter implements Reporter {
  ...
  async onTestRunEnd(testModules: ReadonlyArray<TestModule>, unhandledErrors: ReadonlyArray<SerializedError>): Promise<void> {
    ...
    // Capture module graph data for each test file
...
export async function readBlobs(
  ...
  const promises = blobsFiles.map(async (filename) => {
    ...
    const [version, files, errors, moduleKeys, coverage, executionTime, moduleGraphData] = parsed
    if (!version) {
      throw new TypeError(
    ...
    return { version, files, errors, moduleKeys, coverage, file: filename, executionTime, moduleGraphData: moduleGraphData || {} }
  })
  ...
  // Merge module graph data from all blobs
  const moduleGraphData: Record<string, Record<string, ModuleGraphData>> = {}
  blobs.forEach((blob) => {
    Object.entries(blob.moduleGraphData).forEach(([projectName, projectGraphs]) => {
      moduleGraphData[projectName] ??= {}
      Object.assign(moduleGraphData[projectName], projectGraphs)
    })
  })
