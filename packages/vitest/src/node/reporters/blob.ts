...
export class BlobReporter implements Reporter {
  ...
  async onTestRunEnd(testModules: ReadonlyArray<TestModule>, unhandledErrors: ReadonlyArray<SerializedError>): Promise<void> {
    ...
    await Promise.all(files.map(async (file) => {
      if (file.filepath in sources) {
        return
      }
      try {
        sources[file.filepath] = await readFile(file.filepath, 'utf-8')
      }
      catch {
        // file may not exist on disk (e.g. in-source tests, virtual files)
      }
    }))
...
export async function readBlobs(
  ...
  const promises = blobsFiles.map(async (filename) => {
    ...
    const [version, files, errors, coverage, executionTime, environmentModules, sources] = parse(
      content,
    ) as MergeReport
    if (!version) {
      throw new TypeError(
    ...
    return { version, files, errors, coverage, file: filename, executionTime, environmentModules, sources: sources ?? {} }
  })
  ...
  for (const blob of blobs) {
    for (const [filepath, source] of Object.entries(blob.sources)) {
      if (!sources[filepath]) {
...
export interface MergedBlobs {
  ...
  /**
   * Test source file contents captured on the producer side, keyed by
   * absolute filepath. Lets `--merge-reports` reconstruct the HTML "Code"
   * tab for runs whose source files do not exist on the merger's
   * filesystem (e.g. cross-OS shards). Empty when blobs come from a
   * version that did not record sources.
   */
