...
describe("Issue 8646 - userEvent.upload with file inputs", () => {
  it("fails without unique identifiers (original issue)", async () => {
    // This is the original failing case from the issue
    // When the input has no id, no label, and no data-testid,
    // vitest/browser generates a generic CSS selector (e.g., "html > body > input")
    // which may be ambiguous or fail to locate the element reliably
    ...
    // This may timeout due to ambiguous CSS selector
