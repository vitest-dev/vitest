...
Apply a filtering function to each frame of each stack trace when handling errors. This does not apply to stack traces printed by [`printConsoleTrace`](https://vitest.dev/config/#printconsoletrace). The first argument, `error`, is a `TestError`.
