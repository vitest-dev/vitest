...
Vitest diff rendering also uses [`@vitest/pretty-format`](https://npmx.dev/package/@vitest/pretty-format) under the hood, but the `diff` config only controls the diff-specific layer. Only part of `DiffOptions` is forwarded to the pretty-format configuration, while the rest affects diff rendering itself.
