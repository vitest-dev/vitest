...
Vitest UI also can generate the html report. You can setting the `html` in [reporter options](/config/#reporters). Vitest html report will use the Vitest UI report mode to show the test result. In report mode, Vitest UI will disable the read file content feature, but you can still can see the result of the test with Vitest UI.
