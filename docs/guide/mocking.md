...
Whenever we test code that involves `timeOut`s or intervals, instead of having our tests wait it out or time-out. We can speed up our tests by using "fake" timers by mocking calls to `setTimeout` and `setInterval`, too.
