...
Start Vitest with a `--standalone` flag to keep it in the background. It won't run any tests until they change (Vitest will not run tests if source code is changed until the test that imports the source has been run)
