...
<!-- TODO: slop -->
::: tip
If `--reporter=blob` is used without an output file, the default path will include the current shard config and blob reporter `label` option to avoid collisions with other Vitest processes.
:::
