...
:::tip
It's recommended to always define [`coverage.include`](https://vitest.dev/config/#coverage-include) in your configuration file `vite.config.js`.
...
- enable `coverage.enabled=true` in your configuration file `vite.config.js` or run Vitest with `--coverage.enabled=true` flag
