...
The package exposes two entry points: `vitest-browser-vue` and `vitest-browser-vue/pure`.

By default, `vitest-browser-vue` calls [`cleanup`](#cleanup) before the next test starts so the previous render remains visible in the Browser UI until the test finishes. The `pure` entry point disables this automatic cleanup.
