...
- **Type:** `(name: string | Function, fn: TestFunction, options?: number | TestOptions) => void`
  ...
  `describe.sequential` in a suite marks every tests as sequential. This is useful if you want to run tests in sequential within `describe.concurrent` or with the `--sequence.concurrent` command option.
