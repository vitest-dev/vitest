...
The team has no issue with people using AI as a personal assistant when contributing to Vitest. However, we strongly believe that a real person must be behind every pull request and issue.

All issues and pull requests must be created by a real person using one of the official templates. For the sake of transparency, if a pull request was created with the help of AI, please disclose the tool that was used (for example, Claude, Codex, Copilot, etc).

Pull requests or issues that were entirely generated by AI without any human involvement (for example, by an automated agent) will be marked with a "PLACEHOLDER" label and closed automatically after 3 days, unless a real person gets involved.
